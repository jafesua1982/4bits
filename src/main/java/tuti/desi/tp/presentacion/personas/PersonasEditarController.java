...
public class PersonasEditarController {
	...
	public String preparaForm(Model modelo, @PathVariable("id") Optional<Long> id) {
		if (id.isPresent()) {
			Persona p = servicioPersona.getById(id.get());
			modelo.addAttribute("formBean", new PersonaForm(p));
		} else {
	...
	public String submit(@ModelAttribute("formBean") @Valid PersonaForm formBean,
